...
export default class GameBoard {
  ...
  getActions = (row, col) => {
    ...
    row > 0 &&
      this.grid[row - 1][col] !== C.TREE &&
      actions.push(C.ACTIONS.NORTH)
    row < max &&
      this.grid[row + 1][col] !== C.TREE &&
      actions.push(C.ACTIONS.SOUTH)
    col > 0 &&
      this.grid[row][col - 1] !== C.TREE &&
      actions.push(C.ACTIONS.WEST)
    col < max &&
      this.grid[row][col + 1] !== C.TREE &&
      actions.push(C.ACTIONS.EAST)
    return actions
  }
  ...
  run = reset => {
    ...
    const foxAction = this.foxActor.timestep(
      this.getActions,
      reset,
      this.foxPosition,
    ...
    const chickenAction = this.chickenActor.timestep(
      this.getActions,
      reset,
      this.chickenPosition,
      previousFoxPosition
...
const samePosition = (posA, posB) => posA[0] === posB[0] && posA[1] === posB[1]
